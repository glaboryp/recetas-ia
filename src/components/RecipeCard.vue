...
const saveRecipe = () => {
  ...
  .catch(() => {
    toast.add({ severity: 'error', summary: 'Error', detail: 'Ha ocurrido un error al guardar la información. Por favor, inténtelo en unos minutos', life: 3000 })
  })
}
...
const deleteRecipe = () => {
  ...
  .then(() => {
    toast.add({ severity: 'info', summary: 'Éxito', detail: 'Se ha eliminado la receta de favoritos', life: 3000 })
    emit('changeFavorite', false)
  })
  .catch(() => {
    toast.add({ severity: 'error', summary: 'Error', detail: 'Ha ocurrido un error al eliminar la receta. Por favor, inténtelo en unos minutos', life: 3000 })
  })
}
</script>
